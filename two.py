...
def checksum(data):
    total = 0
    matrix = sep_lines(data)
    for row in matrix:
        diff = find_difference(row)
        total = total + diff
        # print total
    return total

test_data = """5048 177 5280 5058 4504 3805 5735 220 4362 1809 1521 230 772 1088 178 1794
...
# print checksum(test_data)

def sort_lines(matrix):
    for row in matrix:
        row.sort()
        row.reverse()
...
def checksum2(data):
    ...
    for line in matrix:
        for i in range(len(line)):
            dividend = line[i]
            for divisor in line[i+1:]:
                if dividend % divisor == 0:
                    total += dividend / divisor
    return total
            
small_data_2 = """5 9 2 8
9 4 7 3
3 8 6 5"""

print checksum2(test_data)